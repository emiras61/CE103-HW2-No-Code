...
int ce103_fibonacciNumber(int fiIndex)
{
	...
	for (i = 0; i < fiIndex; i++)                        // loops from the first index to the desired index
	{
		if (i <= 1)                                      // to assign the value "1" to the first and second indexes
			result = i;
		else
		{
			result = first + second;                     // adds and discards the previous two indices to find the ith index
			first = second;                              // assigns the previous value to the second previous values
			second = result;                             // assigns the last index to the penultimate index
		}
	}
	return result;                                       // the desired Fibonacci number is sent to the main function.
}
...
{
	int c = 0;                                       // initializing c variable
	while (fiStr[c] != '\0')                         // increment "c" until "null" character
		c++;
...
int ce103_strcmp(const char* fiLhs, const char* fiRhs)
{
	...
	return *(const unsigned char*)fiLhs - *(const unsigned char*)fiRhs;
}
